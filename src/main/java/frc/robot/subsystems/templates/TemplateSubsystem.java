...
//            positionStatusSignal.setUpdateFrequency(50);
    ...
    public void configureFollowerMotor(int followerMotorId, CANBus canbus, boolean opposeMasterDirection) {
...
//        secondaryMotorConfig.MotionMagic.MotionMagicJerk = secondaryJerk;
    ...
    public void configureEncoder(int encoderId, CANBus canbus, double magnetOffset,
    ...
    public void setSecondaryPercent(double percent) {
        followLastMechProfile = false;
        if (percent > 1) percent /= 100d;
        secondaryMotor.setThrottle(percent);
    }
    ...
    public boolean isAboveSpeed(double goalVelocity) {
        if (type != SubsystemType.ROLLER) return false;
    ...
    public double getDegrees() {
        return encoder == null ? positionStatusSignal.getValueAsDouble() * gearRatio * 360d : getEncoderDegrees();
    }
...
//        return BaseStatusSignal.getLatencyCompensatedValueAsDouble(positionStatusSignal, velocityStatusSignal);
    ...
    public double getMechM() {
        if (type != SubsystemType.LINEAR) return 0;
